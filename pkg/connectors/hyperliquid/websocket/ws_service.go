...
func (ws *WebSocketService) subscribeToChannel(channel, coin, interval string, callback func(WSMessage)) (int, error) {
...
func (ws *WebSocketService) parseOrderBook(msg WSMessage) (*OrderBookMessage, error) {
	return ws.parser.ParseOrderBook(msg)
}

func (ws *WebSocketService) parseTrades(msg WSMessage) ([]TradeMessage, error) {
	return ws.parser.ParseTrades(msg)
}

func (ws *WebSocketService) parseKline(msg WSMessage) (*KlineMessage, error) {
...
func (ws *WebSocketService) handleOrderbookMessage(data []byte) error {
	...
	for _, sub := range ws.subscriptions {
		if sub.Channel == "l2Book" {
			msg := WSMessage{Channel: msgWrapper.Channel, Data: msgWrapper.Data}
			sub.Callback(msg)
		}
	}
